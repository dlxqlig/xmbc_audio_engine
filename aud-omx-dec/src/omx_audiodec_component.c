...
void omx_audiodec_component_BufferMgmtCallback(OMX_COMPONENTTYPE *openmaxStandComp, OMX_BUFFERHEADERTYPE* pInputBuffer, OMX_BUFFERHEADERTYPE* pOutputBuffer)
{
	...
	if (got_frame) {
		...
		data_size2 = av_samples_get_buffer_size(&linesize2, omx_audiodec_component_Private->avCodecContext->channels, decoded_frame->nb_samples, omx_audiodec_component_Private->pAudioSampleFormat.sample_fmt, 1);
	}
		...
		if (data_size1 > 0){
			...
			if (!pConvert){
				pConvert = swr_alloc_set_opts(NULL,
						...
						omx_audiodec_component_Private->pAudioSampleFormat.sample_fmt,
		...
		OMX_PTR ComponentParameterStructure)
{
	...
	switch(nParamIndex) {
	case OMX_IndexConfigAudioSampleFormat:
		pAudioSampleFormat = (OMX_AUDIO_CONFIG_SAMPLEFORMATTYPE*) ComponentParameterStructure;
		portIndex = pAudioSampleFormat->nPortIndex;
		err = omx_base_component_ParameterSanityCheck(hComponent,portIndex,pAudioSampleFormat,sizeof(OMX_AUDIO_CONFIG_SAMPLEFORMATTYPE));
		if(err!=OMX_ErrorNone) {
			DEBUG(DEB_LEV_ERR, "In %s Parameter Check Error=%x\n",__func__,err);
			break;
		}
		if (pAudioSampleFormat->nPortIndex <= 1) {
			memcpy(&omx_audiodec_component_Private->pAudioSampleFormat,pAudioSampleFormat,sizeof(OMX_AUDIO_CONFIG_SAMPLEFORMATTYPE));
		} else {
		...
		OMX_PTR ComponentParameterStructure)
{
	...
	case OMX_IndexConfigAudioSampleFormat:
		...
		if ((err = checkHeader(ComponentParameterStructure, sizeof(OMX_AUDIO_CONFIG_SAMPLEFORMATTYPE))) != OMX_ErrorNone) {
			break;
		}
		memcpy(pAudioSampleFormat, &omx_audiodec_component_Private->pAudioSampleFormat,sizeof(OMX_AUDIO_CONFIG_SAMPLEFORMATTYPE));
		break;
	case OMX_IndexParamPortDefinition:
		pPortParam = (OMX_PARAM_PORTDEFINITIONTYPE*) ComponentParameterStructure;
