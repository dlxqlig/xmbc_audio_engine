...
bool OMXPlayerAudio::Open(int channel, int sample_rate, int sample_size, float queue_size, float fifo_size)
{
    m_g_OMX.Initialize();
...
bool OMXPlayerAudio::OpenDecoder()
{
    ...
    bDecoderOpen = m_decoder->Initialize(m_channel, m_sample_rate, 16, 0, m_fifo_size);
...
bool OMXPlayerAudio::SetParameter(PARAMETER_TYPE type, int param)
{
    return m_decoder->SetParameter(type, param);
}

bool OMXPlayerAudio::GetParameter(PARAMETER_TYPE type, int* param)
{
    return m_decoder->GetParameter(type, param);
}
